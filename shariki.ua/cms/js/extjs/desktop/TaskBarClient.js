...
Ext.extend(Ext.ux.TaskBar, Ext.util.Observable, {
    init : function(){
		...
		var clBox = new Ext.BoxComponent({
			el: 'ux-taskbar-client',
	        id: 'TaskBarClientInfo',
	        minWidth: 120,
			region:'east',
			split: false,
			width: 220,
        ...
        var container = new Ext.ux.TaskBarContainer({
			el: 'ux-taskbar',
			layout: 'border',
			items: [sbBox,this.tbPanel,clBox]
		});
    ...
    Ext.ux.TaskBar.TaskButton.superclass.constructor.call(this, {
        ...
        handler : function(){
            if(win.minimized || win.hidden){
                win.show();
            }else if(win == win.manager.getActive() && win.minimizable === true){
                win.minimize();
            }else{
                win.toFront();
            }
...
Ext.extend(Ext.ux.TaskBar.TaskButton, Ext.Button, {
    onRender : function(){
        ...
        this.cmenu.on('beforeshow', function(){
            ...
            var w = this.win;
            items[0].setDisabled((w.maximized !== true && w.hidden !== true) || w.maximizable !== true);
            ...
            items[4].setDisabled(w.closable !== true);
        }, this);
