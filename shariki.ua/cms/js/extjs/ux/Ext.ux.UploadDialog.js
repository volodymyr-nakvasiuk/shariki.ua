...
Ext.ux.UploadDialog.BrowseButton = Ext.extend(Ext.Button, 
{
  ...
  adjustInputFileBox : function()
  {
	if (this.el && this.input_file) { 
		this.input_file.setStyle('font-size', '100px');
		var btn_w, btn_h, inp_w, inp_h;
		btn_w = this.el.getWidth();
		btn_h = this.el.getHeight();
		inp_w = this.input_file.getWidth();
		inp_h = this.input_file.getHeight();
		this.input_file.setLeft((btn_w-inp_w+3)+'px');
		this.input_file.setTop((btn_h-inp_h+3)+'px');
	}
  },
...
Ext.extend(Ext.ux.UploadDialog.Dialog, Ext.Window, {
  ...
  updateToolbar : function()
  {
    ...
    if (this.is_uploading) {
      ...
      //tb.x_buttons.upload.on('render',function(p){
    	//  p.getEl().child(tb.x_buttons.upload.buttonSelector).dom[tb.x_buttons.upload.tooltipType] = this.i18n.upload_btn_start_tip;
      //});
  ...
  onWindowRender : function()
  {
    ...
    this.on('show',function(){
		var tb = this.grid_panel.getBottomToolbar();
		tb.x_buttons.add.adjustInputFileBox();
	});
  },
