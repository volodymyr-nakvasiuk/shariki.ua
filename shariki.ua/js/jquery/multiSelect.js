	...
	$.extend($.fn, {
		multiSelect: function(o, callback) {
			...
			$(this).each( function() {
				...
				id = str_replace('[','_',id);
				id = str_replace(']','',id);
				var html = '<input type="text" readonly="readonly" id="'+id+'_msb_select" class="multiSelect" value="" style="cursor: default;" tabindex='+tabindex+'/>';
				...
				$(select).find('OPTION').each( function() {
					var label=$(this).parents("optgroup").attr("label");
					if (active_label != label){
						active_label = label;
						if (active_label){
							html += '<div class="filter_small_optgroup_label">'+active_label+'</div>';
						}
					}
		...
		multiSelectOptionsShow: function() {
			...
			$(this).next('.multiSelectOptions').css({top: offset.top+$('#site-desktop').scrollTop()+'px', left: offset.left+'px'}).scrollTop(0);
			...
			$(this).next('.multiSelectOptions').hover( function() {
				...
				timer = setTimeout('$("#'+id+'").multiSelectOptionsHide(); $("#'+id+'").unbind("mouseenter mouseleave");', 250);
			});
		},
