...
(function($){
	...
	$.template.helpers = $.template.instance.prototype.helpers = {
		substr : function(o, value, start, length){
			return String(value).substr(start, length);
		},
		condition : function(o, value, true_text, false_text, conditionFunction){
			if (conditionFunction) value = eval('('+conditionFunction+')('+value+')');
			
			if (value=='0' || value=='null' || value=='false')  value = false;
			return value?$.template(true_text).apply(o):$.template(false_text).apply(o);
		},
		split : function(o, value, separator, key){
			value = value.split(separator);
			return value[key]?value[key]:value[0];
		}
	};
	...
	$.extend( $.template.instance.prototype, {
		...
		apply: function(values) {
			var fv = function (prefix, values){
				var nv = new Array();
				for (var i in values){
					if (typeof values[i] == 'object'){
						var tmp = fv(i, values[i]);
						for (var j in tmp) nv[j] = tmp[j];
					}
					else {
						nv[(prefix?(prefix+'.'):'')+i] = values[i];
					}
				}
				return nv;
			};
					...
					if (format) {
						if (format.substr(0, 5) == "this."){
							...
							if (args) {
								...
								args = [values, values[name]].concat(args);
							} else {
								args = [values, values[name]];
							}
