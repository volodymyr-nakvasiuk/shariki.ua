...
class ArOn_Sitemap_Generator{
    ...
    public $sitemapIndexHeader = '<sitemapindex
                                    xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
                                    xsi:schemaLocation="http://www.sitemaps.org/schemas/sitemap/0.9
    ...
    public function addUrls($urlsArray) {
        ...
        foreach ($urlsArray as $url) {
            $this->addUrl(isset ($url['loc']) ? $url['loc'] : null,
                isset ($url['lastmod']) ? $url['lastmod'] : null,
     ...
     */
    public function createSitemap($isMultiSiteMap=false) {
        if (!isset($this->urls))
        ...
        $generatorInfo = '<!-- generator="SimpleSitemapGenerator/'.$this->classVersion.'" -->
                          ...
                          <!-- generated-on="'.date('c').'" -->';
        $sitemapHeader = '<?xml version="1.0" encoding="UTF-8"?>'.$generatorInfo.$this->sitemapHeader;
        $sitemapIndexHeader = '<?xml version="1.0" encoding="UTF-8"?>'.$generatorInfo.$this->sitemapIndexHeader;
        foreach(array_chunk($this->urls,$this->maxURLsPerSitemap) as $sitemap) {
            $xml = new SimpleXMLElement($sitemapHeader);
            foreach($sitemap as $url) {
                $this->addXMLUrl($xml, $url);
            }
        ...
        $k=0;
        if ($isMultiSiteMap){
        	$basePathFiles = scandir($this->basePath);
        	$xmlFiles = array();
        	foreach ($basePathFiles as $file){
        		$pathinfo = pathinfo($file);
        		if ($pathinfo['extension']=='gz' && strpos($file, '.xml.gz')!==false) $xmlFiles[] = $file;
        	}
        	$k=count($xmlFiles);
        }
        if (sizeof($this->sitemaps) > 1 || $isMultiSiteMap) {
            for($i=0; $i<sizeof($this->sitemaps); $i++) {
                $this->sitemaps[$i] = array(
                    str_replace(".xml", sprintf("%03u", $i+$k+1).".xml.gz", $this->sitemapFileName),
                    $this->sitemaps[$i]
                );
            }
            ...
            if ($isMultiSiteMap){
	        	foreach($xmlFiles as $file) {
	                ...
	                $row->addChild('loc',$this->baseURL.htmlentities($file));
