<?php
/**
 * Class work with image file
 *
 */
class ArOn_Crud_Tools_Image {

	static function Resize($src, $dest, $width=85, $height=55, $exact=true, $watermark=false, $mode=1, $watermark_position=array(4), $wmode = 1, $progressive=true) {
		...
		if(!is_array( $watermark_position ))
			$watermark_position = array( $watermark_position );
		...
		if($width === false) $width = $ow;
		if($height === false) $height = $oh;
		...
		if ($watermark) {
			if(file_exists( $watermark ) ){
				...
				$wrw = $w / $res_w; $wrw = $wrw>1?$wrw:1;
				$wrh = $h / $res_h; $wrh = $wrh>1?$wrh:1;
				...
				switch ($wmode){
					...
					case 3:
						$ww = $wow / $r;
						$wh = $woh / $r;
						if ($res_w<$ww && $res_h<$wh) $setWatermark = false;
						break;
				}
				
				if ($setWatermark){
					if($exact && !$zero){
						$kw = ($w>$res_w)?(($w - $res_w) / 2):0;
						$kh = ($h>$res_h)?(($h - $res_h) / 2):0;
						if(in_array(1,$watermark_position)){ //left,top
								$dstX = $kw;
								$dstY = $kh;
								imagecopyresampled ( $idest, $wm_image, $dstX, $dstY, 0, 0, $ww, $wh, $wow, $woh );
						}
		...
		if ($progressive) imageinterlace($idest, true);
